...
Ensure your task types (e.g. `process_payment`) are registered as task definitions in Conductor before starting workflows that use them. The package does not register task definitions; only workflow definitions (e.g. via the DSL). Workers poll Conductor for tasks of the types you register in config.
