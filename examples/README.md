...
Use `$def->register($workflowClient)` to register with Conductor when you have a client (e.g. Laravel `Conductor::workflow()`). Task types (e.g. `validate_order`) must be registered as Conductor task definitions before running the workflow.
